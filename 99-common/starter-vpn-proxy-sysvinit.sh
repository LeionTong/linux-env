...
## VPN and Proxy Management Script with SysVinit integration
...
detect_proxy() {
    if command -v danted &> /dev/null; then
        ...
        readonly PROXY_LOG_FILE="/var/log/danted.log"
        readonly PROXY_ERROR_LOG_FILE="/var/log/danted-error.log"
    elif command -v sockd &> /dev/null; then
        readonly PROXY_PROCESS_NAME="sockd"
        readonly PROXY_CONFIG_FILE="/etc/sockd.conf"
        readonly PROXY_LOG_FILE="/var/log/sockd.log"
        readonly PROXY_ERROR_LOG_FILE="/var/log/sockd-error.log"
    else
...
proxy_stop() {
    ...
    sudo rm -f /var/run/sockd.pid /var/run/danted.pid
    sleep 1
...
proxy_status() {
    ...
    if [[ -f "$PROXY_LOG_FILE" ]]; then
        sudo tail -n 50 "$PROXY_LOG_FILE"
    else
        log_warning "日志文件不存在: $PROXY_LOG_FILE"
    fi
    echo ""
}
